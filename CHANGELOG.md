...
### Fixed
- **`AI 限价单成交但挂单记录缺失（账本已入账）` / `AI 限价卖单成交但无对应持仓` 高频刷屏（根因：三市场共用一个撮合引擎，虚拟成交回调被错投到别的市场）**
  - 根因：每个 `MarketDataService`（CN/HK/US）各自调用 `engine.setVirtualFillHook(...)`，而**引擎是三者共用的**——后注册者覆盖先注册者。抢到钩子的实例会把**所有市场**的虚拟成交记进自己的账本；三个市场名册的 agent id 又完全相同（AI1…AI10），港股 AI9 的成交于是被记到 A 股 AI9 头上：挂单记录当然找不到、持仓当然不存在（连做市商库存也被记到错误市场的做市商账上，真实账目被污染）
  - 修复：钩子**只注册一次**（按引擎对象记录归属），并**按成交标的所属市场路由**（`symbolMarket`：H*=HK / U*=US / 其余 CN）到对应实例；未注册市场宁可漏更新也不做兜底错投（错投 = 改写别的市场账本，比漏更新严重）；回调异常仍吞在回调内，绝不冒泡回撮合
  - 附带修复同类跨市场问题：`pruneExpiredVirtualOrders` 新增可选市场过滤（闭市市场 tickCount 不前进，用别市场的 tick 全量清理会把"还没到期"的虚拟挂单提前删掉 → 虚拟流动性凭空消失）
- **排行榜/赛季榜"赚钱了不显示"（AI 账户盈利在榜上永远是 0.00%）**
  - 根因：榜单直接读 `accounts.totalEquity`，而该字段只在 ①开户 ②账户重置 ③**日终结算** 三处写库——两次日终之间（实时档约 4 小时）所有人的市值重估都不反映
  - 修复：新增 `common/live-equity.ts`（口径与日终结算完全一致：`cash + 持仓市值 + 空头保证金 − 融资负债`），排行榜与赛季榜共用；**估值不完整时严格回退存量值**（无行情 / 有持仓缺任一报价 / 算出非有限值 → 一律不用半套估值，避免把浮盈显示成亏损）；日终落库口径保持不变（它仍是历史快照来源）
  - 顺带消掉赛季榜的 N+1：原先每条 entry 各查一次账户 → 改为批量查询（简化仓储不支持 `In()` 时自动回落逐条，语义不依赖批量）

### 验证
- 后端 `tsc --noEmit` 0 error ｜ build OK ｜ **556/556 全绿**（545 → +11：`phase16-g3-fixes`）
- **真实服务器冒烟（临时库 + 1s tick + 机器人持续交易，跑了 4 个游戏日）**：日志里两类告警 **0 次**（修复前同类日志 31~55 次/次运行）；机器人 25 笔真实委托 / 33 笔真实流水
- **实时重估证据（不依赖请求时序）**：35 秒窗口内数据库里的日终存量 `totalEquity` **一字未变**，而榜上 5 个机器人账户的权益发生变化（例：Alpha/CN → 101942.67）→ 该变化只可能来自盘中市值重估

## [Unreleased] - Phase G-2 机器人玩家（算法盘假人，与真人同权限同榜竞技）

### Added
- **一批算法驱动的"假人玩家"接入游戏**（`backend/src/modules/bots/`）：它们是**真实的 `users` 行**（新增 `users.isBot` 列标记）+ CN/HK/US **三个真实账户**（初始资金与注册路径同口径 10 万），与真人**同一套权限、同一套规则**：
  - 委托**只**走 `OrderService.placeOrder(...)`（与真人 `POST /trading/order` 完全同一条服务路径）→ 休市/集合竞价/涨跌停带/购买力/杠杆/做空券源/T+1/手续费/印花税**一律照办**，被拒就是被拒（机器人没有后门，也不直接写 `orders`/`accounts`/`positions`）
  - 因此它们会**挂在真实盘口**上、与真人**互相成交**（可以抢你的单，也可以被你抢），并出现在**同一张排行榜**（`/api/ranking` 的 `isBot` 条目）与赛季里
  - 名册**幂等**开通：`bot_*` 用户名 + 随机强口令 bcrypt（不可登录、不入日志），重启不重复建号，中途缺失的账户自动补齐
  - 决策复用既有纯函数（`agentRng` 确定性随机 + 特征/方向函数），**同一 (游戏日, tick, 机器人) 决策序列可复现**（可回归、可复盘）
  - 与既有「AI 对手盘」（`/market/ai-opponents`：虚拟账本、不落订单、不进榜）**是两套独立机制**——后者是盘口深度，前者是真玩家
- 环境开关（`.env.example` 已收录）：`BOT_PLAYERS_ENABLED`（默认 `true`，仅严格 `'false'` 关闭）/ `BOT_PLAYERS_COUNT`（默认 6，钳 0~20）/ `BOT_PLAYERS_TRADE_EVERY_TICKS`（默认 10，钳 1~1000，快档必须靠它限流）/ `BOT_PLAYERS_MAX_ORDERS_PER_DAY`（默认 12，钳 0~200，按游戏日重置）
- 排行榜输出新增 `isBot` 字段；机器人条目的 `username` 用**未脱敏展示名**（`bot_alpha` → `Alpha`，机器人无隐私诉求），真人脱敏口径**逐字未变**；前端在全服榜用户名前渲染 🤖 徽标 + 表头一行图例

### Changed
- `MarketService` 追加可选依赖 `@Optional() botPlayers`（参数表末尾，既有构造顺序不动），在 A 股 tick 内、`checkPendingOrders()` **之前**驱动机器人：机器人本 tick 挂的单与真人挂单进同一队列、同一优先级、同一撮合；机器人侧异常**必须**吞掉（假人绝不允许打断行情 tick）

### 验证
- 后端 `tsc --noEmit` 0 error ｜ build OK ｜ 全量 jest **545/545 全绿**（527 → +18：`phase15-bot-players` 17 条 + 赛季默认关闭 1 条）
- 前端 lint 0 error（97 警告）/ `tsc --noEmit` 0 error / 全量 jest **95/95 全绿**（+3：`Ranking.bot.test.tsx`）/ 生产构建（含 PWA 清单门禁）通过 ｜ 浏览器 **E2E 7/7 PASS**
- **启动冒烟（真实 HTTP + 真实 SQLite 临时库，1s tick）**：`ALL PASS` 18 项断言 —— 名册 6 个真实用户 + 18 个真实账户（初始 10 万同口径）、真实委托 11 笔 / 真实流水 18 笔（分笔成交按账户×标的×方向汇总核对股数）、成交单笔笔有流水且费用已计（佣金/印花税字段）、现金/负债/权益非负且有限、持仓非负、杠杆未被动用；`/api/ranking` 同榜 24 条里 18 条机器人（`Alpha/Beta/…` 未脱敏）与真人（`ad***` 脱敏）字段同构、名次混排
- **浏览器证据**：`tests/e2e/artifacts/20260910-204702/04-ranking.png` 经视觉复核 —— 全服榜第 4-7 行是人机混排（1-3 行为脱敏真人，其后为 `Alpha/Alpha/Alpha/Beta`），表头图例「🤖 = 算法盘」在位；赛季区仍是"报名中 · 首位报名者开赛"（机器人未抢跑）

### 已知缺口 / 顺带发现（均进台账）
- **R5-⑬ 的副作用（新发现）**：竞价虚拟成交开始触发钩子后，AI **虚拟**账本出现两类告警（`挂单记录缺失` / `无对应持仓`），对照实验：R5-⑬ 之前的 E2E 日志 0 次，之后 31~55 次 —— 仅影响合成对手盘（不持真实资金，且按设计照记现金继续），但会掩盖真异常；需定性（竞价是否该在钩子前补齐挂单记录 / 是否只对当日新挂单触发）
- 机器人赛季报名默认关闭（首个报名者会替全部真人开赛并关闭报名窗口，冒烟实测），需要"真人先报"判定后再打开
- 赛季榜（默认 Tab）不返回 username/isBot（真人也只显示 `玩家 <id 前6位>`），机器人无法在该榜识别；领奖台未加 🤖
- 名册缩容不回收账号（刻意防误删）；机器人委托未带 `clientOrderId`（建议确定性键以抗 tick 中途重启）
- 管理端统计未区分人机（`admin/stats` 用户数含机器人）
