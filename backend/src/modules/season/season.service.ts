...
export class SeasonService {
    ...
    private settleChain: Promise<unknown> = Promise.resolve();

    constructor(
        @InjectRepository(Season) private readonly seasonRepo: Repository<Season>,
        @InjectRepository(SeasonEntry) private readonly entryRepo: Repository<SeasonEntry>,
        @InjectRepository(Account) private readonly accountRepo: Repository<Account>,
        @Inject(MarketDataService) private readonly marketData: MarketDataService,
        @Inject('MarketDataHK') private readonly marketDataHK: MarketDataService,
        @Inject('MarketDataUS') private readonly marketDataUS: MarketDataService,
    ) {}
    ...
    async ensureSeason() {
        ...
        if (!season) {
            const settled = await this.seasonRepo.find({ where: { status: SeasonStatus.SETTLED } });
    ...
    async enroll(userId: string) {
        ...
        if (season.status !== SeasonStatus.ENROLLING) {
    ...
    async isBlocked(userId: string): Promise<boolean> {
        const season = await this.seasonRepo.findOne({ where: { status: SeasonStatus.RUNNING } });
        if (!season)
            return false;
        const entries = await this.entryRepo.find({ where: { seasonId: season.id, userId, status: EntryStatus.ACTIVE } });
        return entries.length > 0;
    }

    // 实时榜单：报名快照净值口径——合成收益率 = Σ(totalEquity-startEquity) / ΣstartEquity（本金差异免疫）
    async leaderboard(seasonId: string | null, market: string = 'ALL', limit: number = 20) {
        const season = seasonId ? await this.seasonRepo.findOne({ where: { id: seasonId } }) : await this.seasonRepo.findOne({ where: { status: SeasonStatus.RUNNING } });
        if (!season)
            return [];
    ...
    async myStatus(userId: string) {
        ...
        const daysLeft = season.status === SeasonStatus.RUNNING
    ...
    async settleSeasonInner() {
        ...
        const entries = await this.entryRepo.find({ where: { seasonId: season.id, status: EntryStatus.ACTIVE } });
        const byUser = new Map();
        for (const e of entries) {
            const account = await this.accountRepo.findOne({ where: { id: e.accountId } });
    ...
    async history() {
        const settled = await this.seasonRepo.find({ where: { status: SeasonStatus.SETTLED }, order: { seq: 'DESC' }, take: 5 });
        const out = [];
        for (const s of settled) {
            const entries = await this.entryRepo.find({ where: { seasonId: s.id, status: EntryStatus.SETTLED } });
            ...
            // as const 仅为满足 Map 构造泛型的元组推断（纯类型层，运行时无差异）
            const uniq = [...new Map(champs.map((c) => [c.userId, c] as const)).values()];
    ...
    async archive(seasonId: string, userId: string) {
        const season = await this.seasonRepo.findOne({ where: { id: seasonId } });
        if (!season || season.status !== SeasonStatus.SETTLED) {
            return { success: false, error: '赛季不存在或未结算' };
        }
        const entries = await this.entryRepo.find({ where: { seasonId: season.id, status: EntryStatus.SETTLED } });
        const byUser = new Map();
        for (const e of entries) {
    ...
    async points(limit: number = 20) {
        const settled = await this.seasonRepo.find({ where: { status: SeasonStatus.SETTLED } });
        settled.sort((a, b) => Number(b.seq) - Number(a.seq)); // fakeRepo 无 order，服务内排序
        const users = new Set<string>();
        const entryAll: { seasonSeq: number; userId: string; finalRank: number }[] = [];
        for (const s of settled) {
            const es = await this.entryRepo.find({ where: { seasonId: s.id, status: EntryStatus.SETTLED } });
            for (const e of es) {
                users.add(e.userId);
